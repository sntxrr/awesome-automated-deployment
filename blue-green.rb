...
begin
  ...
  puts 'You don\'t have the aws-sdk, optparse or tje pry gem installed, please do one or more of the following'
...
class BlueGreenDeploy
  def initialize(asg_name_prefix, elb_name_prefix, new_ami=nil, region, verbose)
    ...
    @new_ami = new_ami
    @verbose = verbose
    @elb_names = ["#{@elb_name_prefix}", "#{@elb_name_prefix}-vnext"]
  end
  ...
  def get_active_elb
    ...
    active_asg_elbs = @asg_elbs.select { |asg| asg.load_balancer_name }

    raise "ERROR: Unable to find any active ASG ELBs" if active_asg_elbs == nil or active_asg_elbs.first == nil
  ...
  def new_and_existing_amis_match
    ...
    return inactive_launch_config_ami_id == active_launch_config_ami_id
  end
  ...
  def update_launch_config_for_reuse(launch_config, new_launch_config_name)
    ...
    [:launch_configuration_arn, :created_time].each { |k| launch_config.delete(k) }
    filtered_launch_config_options = launch_config.reject { |k, v| v.is_a?(String) && v.empty? }
    ...
    return filtered_launch_config_options
  end
  ...
  def wait_for_elb_capacity
    ...
    elb_capacity_checks = elb_names.map { |x| false }
    asg_instances = []
    elb_instances = []
    loop do
  ...
  def swap_active_asg
    if @starting_inactive_asg.desired_capacity < 1 or @starting_inactive_asg.max_size < 1
...
# SWAP
if @swap_command != "elb" and @swap_command != "asg"
